...
query_update_db(void *data)
{
  ...
  HASH_WALK_DELSAFE(p->qnh, next, qnhn) {
...
static void
query_rt_notify(struct proto *P, rtable *tbl, net *n, rte *new UNUSED, rte *old UNUSED, ea_list *ea UNUSED)
{
  struct query_proto *p = (void *) P;

  if (HASH_FIND(p->qnh, QNH, n->n.pxlen, n->n.prefix))
    return;

  struct query_net_hash_node *qnhn = sl_alloc(p->qnh_slab);
